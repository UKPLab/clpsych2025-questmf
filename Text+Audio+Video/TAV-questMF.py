...
class dds(Dataset):
    def __init__(self,split,data_path,label_path,q_no,missing_files_list):
        ...
        for i in range(len(p_id_list)):
            ...
            self.data.append([txt_list,speech_file,start_times_aud,end_times_aud,vid_file,df_txt,t,score])
...
if __name__ == '__main__':
    ...
    # train(model, train_dataloader, args.tav_checkpoint_path + '-phq' + str(args.question_number) + '-sr-parameters.pt', val_dataloader, epochs=num_epochs, evaluation=True)
    
    # Load trained TAV model
    best_lstm_regressor = lstm_regressor(pretrain_txt_model, pretrain_aud_model, pretrain_vid_model)
    best_lstm_regressor.load_state_dict(torch.load(args.tav_checkpoint_path + '-phq' + str(args.question_number) + '-sr-parameters.pt'))
