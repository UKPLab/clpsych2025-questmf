...
class lstm_regressor(nn.Module):
    def __init__(self):
        ...
        self.self_vid_txt = nn.MultiheadAttention(200, 4,batch_first=True,dropout=0.8)
        self.self_txt_vid = nn.MultiheadAttention(200, 4,batch_first=True,dropout=0.8)
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            if(val_loss > best_val_loss):
                ...
                torch.save(model.state_dict(), tv-ckpt-path)
...
if __name__ == '__main__':
    ...
    # train(model, train_dataloader, args.tv_checkpoint_path+'-sr-parameters.pt', val_dataloader, epochs=num_epochs, evaluation=True)
    
    # Load trained TV model
    best_lstm_regressor = lstm_regressor()
    best_lstm_regressor.load_state_dict(torch.load(args.tv_checkpoint_path+'-sr-parameters.pt'))
