...
```
python TV-questMF.py -s 42 -a 1 -b 0.5 -d_path 'path to data' -l_path 'path to labels' -t_ckpt 'text checkpoint file path' -v_ckpt 'video checkpoint file path' -tv_ckpt 'tv model checkpoint file path' -qno 8 -train -m_files xx yy zz
```
...
```
python TV-questMF-eval.py -s 42 -d_path 'path to data' -l_path 'path to labels' -t_ckpt 'text checkpoint file path' -v_ckpt 'video checkpoint file path' -tv_ckpt 'tv model checkpoint file path' -m_files xx yy zz
```
where xx, yy, zz denote missing/incomplete file numbers.
