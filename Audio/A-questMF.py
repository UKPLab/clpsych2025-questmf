...
class dds(Dataset):
    def __init__(self,split,data_path,label_path,q_no,missing_files_list):
        ...
        for i in range(len(p_id_list)):
            ...
            self.data.append([speech_file,start_times,end_times,score])
...
def train(model, train_dataloader, a_ckpt_path, val_dataloader=None, epochs=10, evaluation=False):
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            val_loss_ccc,val_loss_rmse,val_loss_mae = evaluate(model, val_dataloader)

            if(val_loss_ccc > best_val_loss_ccc):
                best_val_loss_ccc = val_loss_ccc
                torch.save(model.state_dict(), a_ckpt_path)

            # Print performance over the entire training data
            time_elapsed = time.time() - t0_epoch
            
            print(f"{epoch_i + 1:^7} | {'-':^7} | {avg_train_loss:^12.6f} | {val_loss_ccc:^10.6f} | | {val_loss_rmse:^10.6f} | {val_loss_mae:^10.6f} | {time_elapsed:^9.2f}")
            print("-"*70)
        print("\n")
...
if __name__ == '__main__':
    ...
    if args.train_model:
        train(model, train_dataloader, args.audio_checkpoint_path+'-phq'+str(args.question_number)+'-sr16k-parameters.pt', val_dataloader, epochs=num_epochs, evaluation=True)
    
    # Load trained model
    best_lstm_regressor = lstm_regressor()
    best_lstm_regressor.load_state_dict(torch.load(args.audio_checkpoint_path+'-phq'+str(args.question_number)+'-sr16k-parameters.pt'))
