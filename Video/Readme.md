## Video files
 ...
 - V-Total.py: This file is used for training and evaluating the _Total_ framework. It contains the following arguments:
...
```
python V-Total.py -d_path 'path to data' -l_path 'path to labels' -v_ckpt 'video checkpoint file path' -train -m_files xx yy zz
```
...
```
python V-Total.py -d_path 'path to data' -l_path 'path to labels' -v_ckpt 'video checkpoint file path' -m_files xx yy zz
```
where xx, yy, zz denote missing/incomplete file numbers.
...
```
python V-questMF.py -d_path 'path to data' -l_path 'path to labels' -v_ckpt 'video checkpoint file path' -qno 8 -train -m_files xx yy zz
```
where xx, yy, zz denote missing/incomplete file numbers.
...
```
python V-questMF-eval.py -d_path 'path to data' -l_path 'path to labels' -v_ckpt 'video checkpoint file path' -m_files xx yy zz
```
where xx, yy, zz denote missing/incomplete file numbers.
