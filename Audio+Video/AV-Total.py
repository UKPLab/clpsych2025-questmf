...
class lstm_regressor(nn.Module):
    def __init__(self):
        ...
        self.self_aud_vid = nn.MultiheadAttention(200, 4,batch_first=True,dropout=0.8)
        self.self_vid_aud = nn.MultiheadAttention(200, 4,batch_first=True,dropout=0.8)
...
if __name__ == '__main__':
    ...
    if args.train_model:
        train(model, train_dataloader, args.av_checkpoint_path+'-sr-parameters.pt', val_dataloader, epochs=num_epochs, evaluation=True)
    
    # Load trained AV model
    best_lstm_regressor = lstm_regressor()
    best_lstm_regressor.load_state_dict(torch.load(args.av_checkpoint_path+'-sr-parameters.pt'))
