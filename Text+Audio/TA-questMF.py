...
class dds(Dataset):
    def __init__(self,split,data_path,label_path,q_no,missing_files_list):
        ...
        for i in range(len(p_id_list)):
            ...
            self.data.append([txt_list,speech_file,start_times,end_times,score])
...
if __name__ == '__main__':
    ...
    # train(model, train_dataloader, args.ta_checkpoint_path+'-phq'+str(args.question_number)+'-sr16k-parameters.pt', val_dataloader, epochs=num_epochs, evaluation=True)

    # Load trained AT model
    best_lstm_regressor = lstm_regressor(pretrain_txt_model, pretrain_aud_model)
    best_lstm_regressor.load_state_dict(torch.load(args.ta_checkpoint_path+'-phq'+str(args.question_number)+'-sr16k-parameters.pt'))
